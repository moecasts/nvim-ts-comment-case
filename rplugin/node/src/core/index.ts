 ...
 * transform comment from `//` to `/** *\/`
 */
const transformCommentIntoJSDocStyle = (comment: string, indent = '') => {
  if (/\/\*([\s\S]*)\*\//.test(comment)) {
    return comment;
  }
...
export const transform = (content: string, options: TransformOptions = {}) => {
  ...
  forEachComment(src, (fullText, commentRange) => {
    ...
    if (lastCommentRange) {
      const separator = fullText.slice(lastCommentRange.end, commentRange.pos).replace(/^\n[ \t]*/, '');

      const isContinuous = lastCommentRange.kind === ts.SyntaxKind.SingleLineCommentTrivia
        && commentRange.kind === ts.SyntaxKind.SingleLineCommentTrivia
        && (
          !separator ? true : !(separator.split('\n').some(line => {
            return !/[ \t]*\/\//.test(line);
          })));
  ...
  const { transformedContent } = commentRanges.reduce((acc, commentRange) => {
    if (
      (commentCase === CommentCase.JSDoc && commentRange.kind !== ts.SyntaxKind.SingleLineCommentTrivia) ||
      (commentCase === CommentCase.Single && commentRange.kind !== ts.SyntaxKind.MultiLineCommentTrivia)
    ) {
      return acc;
    }
    ...
    if (commentCase === CommentCase.JSDoc) {
      const prefix = fullText.slice(fullText.slice(0, commentRange.pos).lastIndexOf('\n'), commentRange.pos);
      ...
      const indent = /\s*/.test(prefix) ? prefix.match(/\n?([ \t]*)/)?.[1] : '';
